...
- **OpenRaft 依赖升级 v0.10.0-alpha.31 → v0.10.0-alpha.33**: 跟随 aidb 集群存储引擎升级; 上层无 API 使用变化 (`openraft::rt::watch::WatchReceiver` 等路径不变). 获取上游 snapshot 正确性修复与 `rkyv` 漏洞依赖移除. 注意: 单节点 data group 的 `cluster_state` 探活修正见 aidb CHANGELOG (aidb 侧 `LeaderChangeWatcher` self-quorum 判定).
- **GroupSetBatcher 微秒级自适应微退避凑批**: 在 `run_set_batcher` 中引入 `MIN_BATCH_TARGET = 16` 与 `MAX_MICRO_WAIT_US = 50us` 双阶段微退避防单打机制, 消除高并发解冻震荡造成的 1 笔单打提案, SET p50 延迟降低 24.8% (从 11.28ms 降至 8.48ms), SET p99 降低 28.9% (降至 32.03ms), GET 吞吐提升至 35,065.57 req/s. 文件: `src/storage/cluster_adapter.rs`.
- **集群 Data Raft Node 启用 LogCommitter 自适应组提交**: 在 `init_cluster` 阶段为 Data Raft Node 配置 `LogCommitterConfig::default()`，开启日志追加层的自适应非阻塞凑批. 文件: `src/main.rs`.
...
- **全局分配器**: 从 glibc malloc 替换为 mimalloc, 降低多线程分配器争用. 文件: `src/main.rs`, `Cargo.toml`.
...
- **INFO 8.8 字段级对齐**: 支持 `INFO section [section ...]`; `INFO all` = default + commandstats + errorstats + threads + latencystats; `INFO everything` 再加 modules; default 段补齐 Redis 8.8 键名 (无真源字段 stub 0/-1/ok); `errorstats` 按错误前缀 (`ERR`/`WRONGTYPE`); Persistence 键名 `rdb_last_bgsave_time_sec`; 新增 fixture `redis88_info_full_fields.txt`.
- **INFO catalog OTel sync (P3, breaking)**: 热路径仅写 `ServerMetrics`; OTLP counter/histogram 经 `refresh_runtime_metrics` → `sync_otel_from_server_metrics` delta 同步 (相对 INFO 最多滞后 ~15s). 见 `observability-reference.md`.
...
- **集群迁移期间只读副本惰性过期报错**: `KvStorageAdapter` 惰性过期发现 key 已过期时, 之前无条件对底层 `delete()` 发起物理删除; 集群模式下只读副本 (Read + readonly 允许在副本本地 Execute) 上的一次普通 GET 若命中已过期 key, 会触发 `ClusterDataAdapter::delete()` → `propose_group()`, 而副本 propose 必然以 `NotLeader` 失败 (无网络转发), 把本该返回 `nil` 的 GET 变成硬错误. 新增 `StorageAdapter::allow_lazy_expire_delete`, 只有本节点是该 key 所在 data group 的 Raft leader 时才真正发起删除, 副本上直接跳过 (留给 leader 或后续写连接清理). 回归: `tests/cluster_lazy_expire.rs`.
- **slot 迁移期间源侧 ASK 导致读空 (P0)**: `SlotStatus::Migrating` 分支之前对 source_group 本地且非 ASKING 的请求一律返回 `ASK` 到 target, 但内置迁移 executor 只把 key **正向拷贝** (source → target), 从不从 source 删除, 所以在整个 Prepare/Migrating 窗口期间 source 对这批 slot 的所有 key 仍然完全权威, 只有 `CommitSlotMigration` 才会原子切换 slot 归属. 迁移进行中对 source 的读请求返回 `ASK` 会让客户端去查一个可能尚未拷贝到该 key 的 target, 读到"空", 而实际数据一直好好地在 source. 现在 source_group 本地时 (无论 leader 还是只读副本 + readonly) 继续本地 `Execute`, 不再盲目 ASK; 非 leader 且非 readonly 的写请求仍正确 `MOVED` 到 source 的 leader. 回归: `tests/cluster_routing.rs` (`route_decision_all_branches` 7/7b/7c 三个分支).
- **ATOM.EXEC/EXEC JSON batch 快照阶段 MOVED 透传**: 移除透明转发 (`forward_command`) 后, `cmd_atom_exec_json_batch` 的写前快照 (`snapshot_write_keys` → 内部 `DUMP`) 若命中非本地 slot, 会把 `RespValue::Error("MOVED ...")` 包装成 `ERR internal error during batch snapshot: 命令错误: MOVED ...` 冒泡给客户端, 导致 StackExchange.Redis 等集群感知客户端无法识别标准 MOVED/ASK 格式并自动重定向重试整个 batch 直接报错 (TL.KvDoc `KvDocTransaction.CommitAsync()` 复现). 现在快照阶段命中 MOVED/ASK 时直接透传顶层重定向响应 (并补齐此前遗漏的已写入 key 回滚); 新增 `tests/cluster_atom_exec_json_batch_redirect.rs` 回归.
