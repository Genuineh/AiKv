...
impl Metrics {
  pub fn new() -> prometheus::Result<Self> {
    ...
    let kv_used_memory_bytes = prometheus::IntGauge::new(
      "aikv_used_memory_bytes",
      "Current memory usage in bytes",
    )?;
    let kv_used_memory_peak_bytes = prometheus::IntGauge::new(
    ...
    let kv_slow_queries_total = prometheus::IntCounterVec::new(
      Opts::new(
        "aikv_slow_queries_total",
        "Total slow queries by command",
      ),
      &["command"],
    )?;
