...
impl StorageAdapter for AiDbEngine {
  async fn get(&self, key: &[u8]) -> Result<Option<Vec<u8>>> {
    let key = key.to_vec();
    self
      .blocking(move |db| db.get(&key).map_err(|e| Error::Storage(e.to_string())))
      .await
  }
  ...
  async fn flush(&self) -> Result<()> {
    self
      .blocking(move |db| db.flush().map_err(|e| Error::Storage(e.to_string())))
      .await
  }
  ...
  async fn close(&self) -> Result<()> {
    self
      .blocking(move |db| db.close().map_err(|e| Error::Storage(e.to_string())))
      .await
  }
