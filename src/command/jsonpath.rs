...
impl JsonPathEngine {
  ...
  pub fn append(&self, json: &mut JsonValue, parts: &[String], values: &[JsonValue]) -> Result<()> {
        ...
        if let JsonValue::Array(arr) = json {
          if let Some(first_elem) = arr.first_mut() {
            ...
            if let JsonValue::Object(obj) = first_elem {
              ...
              let clean_key = target_key.trim_start_matches('[').trim_end_matches(']');
    ...
    if let JsonValue::Array(arr) = json {
      if let Some(first) = parts.first() {
        ...
        if first.starts_with("[?(") && first.ends_with(']') {
          ...
          for item in arr.iter_mut() {
            let matches = self
              .filter_jsonpath(std::slice::from_ref(item), &first[1..first.len() - 1])?
              .is_empty();
            if !matches {
