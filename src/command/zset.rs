...
impl ZSetCommands {
  ...
  async fn rank(&self, db: usize, key: &Bytes, member: &Bytes, reverse: bool) -> Result<RespValue> {
  ...
  async fn bzpop_common(&self, db: usize, args: &[Bytes], max: bool) -> Result<RespValue> {
    ...
    let timeout_s = crate::command::list::ListCommands::parse_timeout_secs(&args[args.len() - 1])?;
