    ...
    if let Some(key) = crate::cluster::forward::cluster_routing_key(cmd, args) {
      ...
      return match decision {
        ...
        crate::cluster::router::RouteDecision::Moved {
          ...
          match crate::cluster::forward::forward_command(&connect_addr, false, cmd, args).await {
        ...
        crate::cluster::router::RouteDecision::Ask {
          ...
          let connect_addr = crate::cluster::state::CLUSTER_STATE_MGR
            ...
            .unwrap_or_else(|| addr.clone());
          match crate::cluster::forward::forward_command(&connect_addr, true, cmd, args).await {
            Ok(resp) => Some(Ok(resp)),
            Err(_) => Some(Ok(RespValue::Error(format!("ASK {slot} {addr}")))),
          }
        }
        crate::cluster::router::RouteDecision::ClusterDown(msg) => Some(Ok(RespValue::Error(msg))),
      };
    }
    None
  }
...
fn classify_command(cmd: &str) -> crate::cluster::router::CommandType {
  match cmd.to_ascii_lowercase().as_str() {
    "get" | "exists" | "hget" | "hgetall" | "hkeys" | "hvals" | "hlen" | "hexists" | "lrange"
    | "lindex" | "llen" | "smembers" | "scard" | "sismember" | "zrange" | "zcard" | "zscore"
    | "zrank" | "type" | "ttl" | "pttl" | "strlen" | "getbit" | "getrange" | "mget"
    | "json.get" => crate::cluster::router::CommandType::Read,
