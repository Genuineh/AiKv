...
impl JsonCommands {
  ...
  pub async fn json_set(&self, db: usize, args: &[Bytes]) -> Result<RespValue> {
    ...
    let result = async {
      ...
      let new_value: JsonValue = serde_json::from_str(&value_str).map_err(Self::invalid_json)?;

      let result_json = if path == "$" || path == "." {
        new_value
      } else {
        let mut json_doc = match self.storage.get(db, key).await? {
          Some(existing) => serde_json::from_slice(&existing).map_err(Self::invalid_json)?,
          None => json!({}),
        };
  ...
  pub async fn json_del(&self, db: usize, args: &[Bytes]) -> Result<RespValue> {
    ...
    let result = async {
      ...
      let mut json_val: JsonValue = serde_json::from_slice(&raw).map_err(Self::invalid_json)?;
  ...
  pub async fn json_update(&self, db: usize, args: &[Bytes]) -> Result<RespValue> {
    ...
    let result = async {
      ...
      if !where_path.is_empty() && !matches!(where_path.as_str(), "$" | "." | "$[*]") {
        ...
        let json_check: JsonValue = serde_json::from_slice(&raw).map_err(Self::invalid_json)?;
  ...
  pub async fn json_mset(&self, db: usize, args: &[Bytes]) -> Result<RespValue> {
    ...
    let result = async {
      ...
      for chunk in args.chunks(3) {
        ...
        let new_value: JsonValue = serde_json::from_str(&val_str).map_err(Self::invalid_json)?;
