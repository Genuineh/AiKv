...
impl ListCommands {
  ...
  pub(crate) fn parse_timeout_secs(b: &Bytes) -> Result<f64> {
    let s =
      std::str::from_utf8(b).map_err(|_| Error::Command("ERR timeout is not a float".into()))?;
    s.parse::<f64>()
  ...
  async fn try_pop_any(&self, db: usize, keys: &[&Bytes], left: bool) -> Result<Option<RespValue>> {
    for &key in keys {
      let Some(mut stored) = self.storage.get_typed(db, key).await? else {
        continue;
      };
