...
impl ClusterRouter {
  ...
  pub fn decide(key: &[u8], cmd_type: CommandType, asking: bool, readonly: bool) -> RouteDecision {
    ...
    match status {
      ...
      SlotStatus::Migrating(source_group) => {
        ...
        let target_group = match &mig_state {
          ...
          None => {
            return RouteDecision::ClusterDown("CLUSTERDOWN migration state not found".into())
          }
        };
      ...
      SlotStatus::Unallocated => {
        ...
        match mgr.router.route_key(key) {
          Ok((group_id, SlotStatus::Assigned(_))) => {
            if mgr.multi_raft.is_group_local(group_id) && mgr.is_local_group_leader(group_id) {
              RouteDecision::Execute
            } else {
              leader_moved(mgr, group_id, slot)
            }
