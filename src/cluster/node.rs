...
impl ClusterNode {
    ...
    pub async fn initialize(&mut self) -> Result<()> {
        ...
        if self.config.is_bootstrap {
            multi_raft.initialize_meta_cluster(self.config.initial_members.clone()).await
    ...
    pub async fn create_data_groups(&self) -> Result<()> {
        let multi_raft = self.multi_raft.as_ref()
            .ok_or_else(|| crate::error::AikvError::Internal("MultiRaftNode not initialized".to_string()))?;
        ...
        for group_id in 1..=self.config.num_groups as u64 {
            ...
            multi_raft.create_raft_group(group_id, replicas).await
                .map_err(|e| crate::error::AikvError::Internal(
                    format!("Failed to create group {}: {}", group_id, e)
                ))?;
        }
    ...
    pub async fn shutdown(&self) -> Result<()> {
        if let Some(multi_raft) = &self.multi_raft {
            multi_raft.shutdown().await
                .map_err(|e| crate::error::AikvError::Internal(format!("Failed to shutdown: {}", e)))?;
        }
        Ok(())
    }
}
