...
pub async fn cluster_rebalance() -> Result<String, String> {
  ...
  let mut group_slots: std::collections::HashMap<u64, Vec<u16>> = std::collections::HashMap::new();
...
pub async fn dispatch_cluster(
  ...
  match sub {
        ...
        .map(|range| {
          let mut entry = vec![
            ...
            RespValue::Array(Some(vec![
              ...
              RespValue::BulkString(Some(Bytes::from(format!("{:040x}", range.master.node_id)))),
            ])),
          ];
          // 追加 replica 条目
    ...
    Some("set-config-epoch") | Some("SET-CONFIG-EPOCH") => Ok(RespValue::SimpleString("OK".into())),
