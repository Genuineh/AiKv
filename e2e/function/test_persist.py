...
编排 (人/部署侧操作, 非单条用例内完成):
- 跑 seed 写入
- 优雅停再起 → 设 `WIKV_E2E_AFTER_RESTART=1` 跑再读
- kill 探针写入 → `kill -9` 再起 → 设 `WIKV_E2E_AFTER_KILL=1` 跑再读
"""
...
def test_persist_after_graceful_restart(dut):
    """优雅重启后再读 (需 WIKV_E2E_AFTER_RESTART=1).
...
def test_persist_after_kill(dut):
    """kill -9 恢复后再读 (需 WIKV_E2E_AFTER_KILL=1).
