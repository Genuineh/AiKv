...
async fn test_rename() {
  ...
  assert_eq!(
    resp,
    aikv::protocol::RespValue::BulkString(Some(b("v")))
  );
}
...
async fn test_type_command() {
  ...
  assert_eq!(
    t1,
    aikv::protocol::RespValue::SimpleString("string".into())
  );
  let t2 = r.execute("TYPE", &[b("l")], &mut db).await.unwrap();
  assert_eq!(
    t2,
    aikv::protocol::RespValue::SimpleString("list".into())
  );
  let t3 = r.execute("TYPE", &[b("missing")], &mut db).await.unwrap();
  assert_eq!(
    t3,
    aikv::protocol::RespValue::SimpleString("none".into())
  );
}
