...
#[cfg(feature = "cluster")]
mod cluster_tests {
    use aikv::cluster::{ClusterCommands, ClusterConfig, ClusterNode, MetaRaftNode, MultiRaftNode, Router};
    ...
    async fn test_meta_raft_add_node_sync() -> Result<()> {
        ...
        // Test: Add a new node via MetaRaft
        meta1.add_node(2, "127.0.0.1:50052".to_string()).await
            .map_err(|e| aikv::error::AikvError::Internal(e.to_string()))?;
        
        // Wait for replication
        sleep(Duration::from_millis(300)).await;
        
        // Verify: Check that the node appears in cluster meta
        let cluster_meta = meta1.get_cluster_meta();
        assert!(cluster_meta.nodes.contains_key(&2), "Node 2 should be in cluster meta");
        assert_eq!(cluster_meta.nodes.get(&2).unwrap().addr, "127.0.0.1:50052");
        
        // Cleanup
        let _ = tokio::fs::remove_dir_all("/tmp/test_cluster_node1").await;
    ...
    async fn test_cluster_meet_metadata_sync() -> Result<()> {
        ...
        let cluster_meta = meta1.get_cluster_meta();
        assert!(cluster_meta.nodes.contains_key(&2), "Node 2 should be in cluster after MEET");
    ...
    async fn test_cluster_addslots_raft_sync() -> Result<()> {
        ...
        for slot in 0..=100 {
            ...
            assert_eq!(assigned_group, 1, "Slot {} should be assigned to group 1", slot);
        }
    ...
    async fn test_cluster_info() -> Result<()> {
        ...
        assert!(info_str.contains("cluster_slots_assigned:"), "Info should contain slots_assigned");
    ...
    async fn test_cluster_nodes() -> Result<()> {
        ...
        assert!(nodes_str.contains("myself"), "Nodes output should mark myself");
    ...
    async fn test_cluster_node_init() -> Result<()> {
        ...
        assert!(node.meta_raft().is_some(), "Meta raft should be initialized");
        assert!(node.multi_raft().is_some(), "Multi raft should be initialized");
